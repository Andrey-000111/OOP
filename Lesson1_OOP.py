# Классы и наследования. Метод (функция) и атрибуты(переменные)
import random
...
class KiaRio(Kia):
    def __init__(self, year=2000, color=(0, 100, 0)):
...
if __name__ == '__main__':
    ...
    kia_rio = KiaRio()
    print(kia_rio)
    print(issubclass(KiaRio, Kia))  #issubclass проверяет является ли класс подклассом другого класса
    print(issubclass(KiaRio, Car))
    print(isinstance(kia_rio, KiaRio))
    print(isinstance(kia_rio, Kia))
    print(isinstance(kia_rio, Car)) #isinstance проверяет является объект объектом этого класса